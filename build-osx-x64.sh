...
cp target/filtered-resources/Info.plist native-osx/StakeScape.app/Contents

echo Setting world execute permissions on StakeScape
pushd native-osx/StakeScape.app
chmod g+x,o+x Contents/MacOS/StakeScape
popd

codesign -f -s "${SIGNING_IDENTITY}" --entitlements osx/signing.entitlements --options runtime native-osx/StakeScape.app || true

# create-dmg exits with an error code due to no code signing, but is still okay
# note we use Adam-/create-dmg as upstream does not support UDBZ
create-dmg --format UDBZ native-osx/StakeScape.app native-osx/ || true

mv native-osx/StakeScape\ *.dmg native-osx/StakeScape-x64.dmg

if ! hdiutil imageinfo native-osx/StakeScape-x64.dmg | grep -q "Format: UDBZ" ; then
    echo "Format of resulting dmg was not UDBZ, make sure your create-dmg has support for --format"
    exit 1
fi

# Notarize app
if xcrun notarytool submit native-osx/StakeScape-x64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
    xcrun stapler staple native-osx/StakeScape-x64.dmg
fi
